...
class S(BaseHTTPRequestHandler):
    base_static_file_path = os.path.dirname(
        os.path.abspath(__file__))+'/static_files'
    ...
    def get_static_file_list(self, path):
        ...
        files = files + [os.path.join(relative_path, cur_path)
    ...
    def do_GET(self):
        ...
        if "/fileserve" in self.path:
            ...
            file_path = urllib.parse.unquote(
                path.replace("/fileserve", "").strip()).split("/")
            list_path = self.base_static_file_path
            if file_path:
                abs_file_path = os.path.join(
                    self.base_static_file_path, *file_path)
                if Path(abs_file_path).is_dir():
                    ...
                    return
            print(
                f"file_path {file_path} base_static_file_path {self.base_static_file_path} list_path {list_path}")

            file_list_response = "<html><ul>" + \
                ''.join(self.get_static_file_list(list_path))+"</ul></html>"
            self._set_headers()
        ...
        if "?" in self.path:
            ...
            currTimestamp = datetime.datetime.now().replace(
            ...
            if int(currTimestamp) - int(reqTimestamp/1000) < 3:
                ...
                subprocess.call("xdotool "+cmd + " " + key,
                                shell=True, timeout=0.1)

        #self.wfile.write(self.getHtmlContent()%{"cmd":cmd, "key":key})
        formatted_string = self.getHtmlContent().replace(
            "%(cmd)s", cmd).replace("%(key)s", key)
    ...
    def transformQueryParam(self):
        ...
        queryParams = {item[0]: item[1]
                       for item in urllib.parse.parse_qsl(queryString)}
        print(queryParams)
        return queryParams
...
def run(server_class=HTTPServer, handler_class=S, addr="0.0.0.0", port=4443):
    ...
    httpd.socket = ssl.wrap_socket(httpd.socket, server_side=True, certfile='./certificate.crt',
                                   keyfile="./privatekey.key", ssl_version=ssl.PROTOCOL_TLS)
